...
active_opps as (
    ...
    from {{ ref("stg_salesforce__opportunities") }} as opp
    left join
        {{ ref("stg_salesforce__accounts") }} as acc
    ...
    left join
        ...
        on opp.opportunity_id = qty.opportunity_id
    left join
        {{ ref("stg_salesforce__record_types") }} as typ
...
valid_leads as (
    ...
    where
        ...
        and lea.status in (
            'Unqualified this Season', 'New', 'Working', 'Nurture'
        )
